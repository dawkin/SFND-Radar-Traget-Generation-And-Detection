...
for i=1:L         
    ...
    tau = range * 2 / c;
    delayed_t = t(i) - tau;
    Rx(i)  = cos(2* pi * ((fc * delayed_t) + (slope * delayed_t^2) /2));
...
Gr = 4;
Gd = 2;
% compute number of training cells
Tcells_size = (((2*Tr)+(2*Gr)+1)*((2*Td)+(2*Gd)+1))-(((2*Gr)+1)*((2*Gd)+1));  
